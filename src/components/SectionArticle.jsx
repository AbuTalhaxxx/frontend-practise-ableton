function SectionArticle({number}) {
	const sections = {
  one: [
    'We make <span class="text-[#0000ff]">Live</span>, ' +
      '<span class="text-[#0000ff]">Push</span> and ' +
      '<span class="text-[#0000ff]">Link</span> — unique ' +
      'software and hardware for music creation and performance. ' +
      'With these products, our community of users creates amazing things.', " Ableton was founded in 1999 and released the first version of Live in 2001. Our products are used by a community of dedicated musicians, sound designers, and artists from across the world."
  ],
   two: [
   ...
   three: [
    ...
    'Rather than having a one-size-fits-all process, we try to give our people ' +
      ...
      'We don’t always agree with each other, but opinion and debate are ' +
...
six: [
	...
	return (
		<article className="mx-12 my-16 md:mx-44 md:my-24 xl:mx-80 xl:my-28">
			<Paragraphs paras={sections[number]}/>
		</article>
	);
}
...
const classPropsOne = "font-bold text-lg lg:text-3xl";

const classPropsTwo = "text-sm mt-5 lg:text-base";

return paras.map((item, index)=>{

return (index === 0) ? (<p className={classPropsOne} dangerouslySetInnerHTML={{__html: item}}></p>): (<p className={classPropsTwo}>{item}</p>);
